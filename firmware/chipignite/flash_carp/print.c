...
void print_format(const char** format_str, va_list args) {
  ...
  while (!format_str_end) {
    ...
    switch (**format_str) {
      ...
      case 'h': num_bytes = 1; break; // print an 8-bit value
      case 'l': num_bytes = 2; break; // print a 16-bit value
      case 'q': num_bytes = 8; break; // print a 64-bit value
...
void print_str(char* s) {
  for (char* c = s; *c != 0; c++) { print_char(*c); }
}
