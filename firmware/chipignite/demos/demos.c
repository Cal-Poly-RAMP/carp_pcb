...
void write_to_ram(uint8_t addr, uint32_t data) { OPENRAM(addr << 2) = data; }

uint32_t read_from_ram(uint8_t addr) { return OPENRAM(addr << 2); }
...
void main() {
  ...
  for (i = 0; i < 16; i++) { write_to_ram(i, i); }
  for (i = 0; i < 16; i++) {
